        ...
        (payment-methods/create
          {:type "card"
           :card {:number    "4242424242424242"
                  ...
                  :exp_year  "2026"
                  :cvc       "314"}} api-key)
        _attached-payment-method
        (customers/attach-payment-method
          customer-id
          (:id payment-method)
          api-key)
        product
        (products/create
          {:name (str "test_product_" (random-uuid))}
          api-key)
    ...
    (testing "should be a valid subscription"
      ...
      (testing "latest-invoice should include payment-intent"
        (let [payment-intent
          ...
          (is (= (:payment_method_types payment-intent) ["bacs_debit" "card"]))
...
(deftest list-subscriptions
  (let [customer (customers/create api-key)
    ...
    (testing "should retrieve a list of subscriptions"
      ...
      (is (some #(= (:id subscription) (:id %))
            (:subscriptions retrieved-subscriptions))))))
